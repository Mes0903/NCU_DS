...
struct Node {
    ...
    int val;
    Node() : row(), col(), val() {}
    Node( const int &row, const int &column, const int &value ) : row( row ), col( column ), val( value ) {}
    ...
    while ( a_index < a[0].val + 1 ) {
        ...
        while ( b_index < b[0].val + 2 ) {
            if ( a[a_index].row != curr_row ) {
                ...
                a_index = row_begin;
                while ( b[b_index].row == curr_col ) ++b_index;
        ...
        while ( a[a_index].row == curr_row ) ++a_index;
