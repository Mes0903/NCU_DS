    ...
    class Linked_List {
      public:
        ...
        void pop_front() {
            ...
            head = std::move( temp->next );    // 把 head 指到剛剛的下一項，也就是說往後 head 往後了一格
        }
        void pop_back() {
            if ( head == nullptr )
                return;
            ...
            Node *current = head.get();
            while ( true ) {
                if ( current->next->next.get() == nullptr )    // 下一個如果是最後一個了，那就要把 next 拿掉
                    break;
        ...
        void insert( int index, int data ) {
            ...
            while ( true ) {
                if ( current != nullptr ) {    // 如果 List 非空
                    if ( cnt + 1 == index ) {    // 如果下一個是要找的元素，就插在現在這個的後方
                ...
                if ( current->next == nullptr ) {    // 如果下一個是空的，那代表 index 不存在
                    return;
                }
                ...
                if ( current != nullptr ) {    // 如果 List 非空
                    ...
                    if ( cnt + 1 == index ) {
                        if ( current->next->next == nullptr )
                            ...
                            current->next = std::move( current->next->next );    // 如果下一個是要找的，那就直接把後面那個元素接過來
        ...
        void clean() {
            while ( head != nullptr ) {
                head = std::move( head->next );    // 逐一刪掉 Node，直接刪的話會因為 destructor 造成 stackoverflow
            }
        }
        void reverse() {
